...
| `FLOOR-0` | Floor trapezoid, depth band 0→1 (nearest) | Yes | **Done** — `assets/tiles/floor_depth0.png` |
| `FLOOR-1` | Floor trapezoid, depth band 1→2 | Yes | **Done** — `floor_depth1.png` |
| `FLOOR-2` | Floor trapezoid, depth band 2→3 | Yes | **Done** — `floor_depth2.png` |
| `FLOOR-3` | Floor trapezoid, depth band 3→4 (farthest) | Yes | **Done** — `floor_depth3.png` |
...
**Only 4 new images needed per surface, done via Option A (Python perspective warp).** `WALL-SIDE-0` through `WALL-SIDE-3` were generated by `tools/generate_wall_side_assets.py`, which takes `wall_plain_01.png`, tiles it a few times, and warps it with Pillow's perspective transform onto each depth band's exact trapezoid shape (using the same `RECT_SIZES` geometry as `game.js`, copied into the script on purpose — see the script's docstring). The right wall doesn't need a separate warp — it's the same image mirrored horizontally, which the script also generates (`*_mirrored.png`). Tried first because it was fast and reused art we already had; the result had genuinely converging brick coursing, so there was no need to fall back to generating fresh art via the image-gen AI.

`FLOOR-0` through `FLOOR-3` follow the exact same approach via `tools/generate_floor_assets.py`, just rotated — a floor piece tapers top-to-bottom (full width near the player, narrower farther away) rather than left-to-right. Uses `floor_plain_01.png`, cropped from a real generated floor image (the flat plain-flagstone area only — the source image also had a decorative brick trim/border that wasn't meant to repeat as a tile, so that part was left out). Ceiling will get the same treatment once ceiling art exists.
